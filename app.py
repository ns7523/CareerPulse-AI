...
class ResumeApp:
    ...
    def load_local_image(self, image_path: str):
        """Load and return an image (JPEG/PNG) from a local file"""
        try:
    ...
    def add_footer(self):
        ...
        <p style='text-align: center; font-size:14px;'>
            🌐 Multi-language support enabled. Choose your language from the top menu.</p>
            """, unsafe_allow_html=True)
            ...
            <p style='text-align: center;'>
                Powered by <b>N S</b> and <b>Google Gemini AI</b> |  2025-2026
    ...
    def render_builder(self):
        ...
        #st.title(t("Resume Builder 📝"))
        #st.write(t("Create your professional resume"))
        page_header(
            "Resume Builder 📝",
    ...
    # Put code and phone on same row (small width for code)
            code_col, phone_col = st.columns([1, 3])
    # try to pre-select code if existing_phone already has one
            existing_number_only = existing_phone or ""
            preselected_index = 0
            if existing_number_only:
                parts = existing_number_only.strip().split()
                if parts and parts[0].startswith("+") and parts[0] in country_codes:
                    preselected_index = country_codes.index(parts[0])
                    existing_number_only = " ".join(parts[1:])  # keep rest as number

            with code_col:
                if 'numeric_phone' not in st.session_state:
    # Strip country code from existing_phone if any
                    if existing_phone:
                        parts = existing_phone.strip().split()
                        st.session_state.numeric_phone = " ".join(parts[1:]) if len(parts) > 1 else existing_phone
                    else:
                        st.session_state.numeric_phone = ""
...
# Extract only the numeric code, e.g., "+91"
                selected_code = selected_display.split("(")[-1].replace(")", "").strip()

                

            #with phone_col:
                #phone_number = st.text_input("Phone", value=existing_number_only, key="phone_input")

            #phone = st.text_input("Phone",value=existing_phone)
            phone = st.text_input("Phone", value=st.session_state.numeric_phone, key="phone_input")
            st.session_state.numeric_phone = phone
            full_phone = f"{selected_code} {st.session_state.numeric_phone}".strip()
    ...
    "Portfolio Website", value=existing_portfolio)
        ...
        st.session_state.form_data['personal_info']['phone'] = full_phone
        

        # Professional Summary
        st.subheader("Professional Summary")
        summary = st.text_area("Summary", value=st.session_state.form_data.get('summary', ''), height=150,
                ...
                try:
                    ...
                    if resume_buffer:
                        try:
                            ...
                            st.success(f"✅ Resume generated successfully! in {language}")

                            # Show snowflake effect
                            st.snow()

                            st.download_button(
                                label=f"📥 Download Resume ({language})",
                                data=resume_buffer,
                                file_name=f"{current_name.replace(' ', '_')}_resume_{language}.docx",
     ...
     "124852522.jpeg")
        ...
        st.markdown("""
            <div class="hero-section">
                <h1 class="hero-title">About CareerPulse AI</h1>
        ...
        st.markdown("""
            <div class="vision-section">
                ...
                <p class="vision-text">
                    "CareerPulse AI represents my vision of democratizing career advancement through technology.
    ...
    def render_home(self):
       ...
       # Hero Section
        hero_section(
            ...
            subtitle="Next-Gen AI Resume Optimizer and Career Guidance System",
            description="Elevate your career with AI-powered resume analysis and creation. Gain personalized insights and craft standout professional resumes tailored to your goals."
        )
...
"""
    ...
    def main(self):
        ...
        with st.sidebar:
            image = Image.open("assets/sidebar_image.jpg")  # 👈 your local image path
